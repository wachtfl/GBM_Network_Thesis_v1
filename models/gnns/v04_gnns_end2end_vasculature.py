...
from sklearn.metrics import roc_auc_score, average_precision_score, roc_curve, balanced_accuracy_score, f1_score
...
def create_data_splits(graph_files, metadata, test_indices, val_ratio=0.2):
    ...
    for i, meta_entry in enumerate(metadata):
        ...
        for graph_file in graph_files:
            ...
            file_patient_id = filename.split('_v2.h5')[0]
...
def run_complete_analysis(metadata_file, graph_pattern, output_dir='gnn_analysis_results', cv_folds=None):
    ...
    for graph_file in graph_files:
        ...
        filename = os.path.basename(graph_file)
        patient_id = filename.split('_v2.h5')[0]  # Remove the suffix
    ...
    if cv_folds is None:
        ...
        labels = [meta['label'] for meta in available_metadata]
        skf = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
        fold_indices = [(test_idx, f"Fold_{i}") for i, (_, test_idx) in enumerate(skf.split(range(len(available_metadata)), labels))]
        print(f"Running stratified {cv_folds}-fold cross-validation...")
    ...
    # Add metrics robust to class imbalance
    patient_balanced_acc = balanced_accuracy_score(patient_labels, patient_predictions > 0.5)
    patient_f1 = f1_score(patient_labels, patient_predictions > 0.5)
    ...
    unique_labels, label_counts = np.unique(patient_labels, return_counts=True)
    print(f"Class Distribution:")
    for label, count in zip(unique_labels, label_counts):
        print(f"  Class {label}: {count} samples ({count/len(patient_labels)*100:.1f}%)")
    
    print(f"\nPatient-Level Metrics:")
    print(f"Overall AUC: {patient_auc:.3f}")
    print(f"Average Precision: {patient_ap:.3f}")
    print(f"Balanced Accuracy: {patient_balanced_acc:.3f}")
    print(f"F1 Score: {patient_f1:.3f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, required=True, 
                       help='Path to directory containing metadata2.json and *_v2.h5 graph files.')
    ...
    graph_pattern = os.path.join(args.data_dir, '*_v2.h5')
